...
function _createTokenList(){
	...
	$ruta = $GLOBALS['rutaDestino'].'/'.$GLOBALS['isbn'].'-'.date('Y-m-d').'.csv';  
		if(file_exists($ruta))
	{
		unlink($ruta);
	}
	...
	fputcsv($fh, array(t('Token'), t('QR')));

	foreach ($result as $row) {
		fputcsv($fh, array($row->token, $row->application_url));
	}
	fclose($fh);

db_update('sso_token')
	->fields(array('status'=>3))
	->condition('isbn',$GLOBALS['isbn'],'=')
	->condition('status',0,'=')
	->execute();
}
