			...
			$.ajax({
				...
				success:function(resp){
					if(resp.code == 200){
						alert(resp.mensaje);
						<?php global $base_url?>
						window.location="<?php echo $base_url;?>/group-sso";
					}else{
						alert(resp.mensaje);
					}
				}
...
<div class="cont_token_form">
	...
	<div class="left_col">
		<?php print drupal_render($form['group']['gid'])?>
		<div class="campos">
			<?php print drupal_render($form['group']['name'])?>
		</div>
		<div class="campos">
			<?php print drupal_render($form['group']['niveles'])?>
		</div>
		<div class="campos">
			<?php print drupal_render($form['group']['grados'])?>
		</div>
	</div>
	<div class="">
